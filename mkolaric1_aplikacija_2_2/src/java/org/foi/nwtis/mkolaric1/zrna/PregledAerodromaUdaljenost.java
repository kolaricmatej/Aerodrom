 ...
 * @author Matej
 */
@Named(value = "pregledAerodromaUdaljenost")
@SessionScoped
public class PregledAerodromaUdaljenost implements Serializable {
    ...
    private String pocetniIcao;
    private String krajniIcao;
    private AvionLeti odabraniLet;
    private double udaljenost;

    /**
     * Creates a new instance of PregledAerodromaUdaljenost
     */
    public PregledAerodromaUdaljenost() {
        air = new AIRPREST();
    }

    public double getUdaljenost() {
        udaljenost=izracunajUdaljenosti();
        return udaljenost;
    }
    ...
    public List<AvionLeti> dohvatiLetove() {
        ...
        listaLetova = podaciOAvionima(a.getIcao(), korime, loz, odVremena, doVremena);
        return listaLetova;
    }

    public double izracunajUdaljenosti() {
        if(odabraniLet!=null){
        AvionLeti a = odabraniLet;
        FacesContext fc = FacesContext.getCurrentInstance();
        String korime = (String) fc.getExternalContext().getSessionMap().get("korisnickoIme");
        String loz = (String) fc.getExternalContext().getSessionMap().get("lozinka");
        double udaljenostA = dodatnoUdaljenostAerodroma(a.getEstDepartureAirport(), a.getEstArrivalAirport(), korime, loz);
        return udaljenostA;
        }
        return 0;
    }
    ...
    public String getPocetniIcao() {
        return pocetniIcao;
    }

    public void setPocetniIcao(String pocetniIcao) {
        this.pocetniIcao = pocetniIcao;
    }

    public String getKrajniIcao() {
        return krajniIcao;
    }

    public void setKrajniIcao(String krajniIcao) {
        this.krajniIcao = krajniIcao;
    }
    ...
    private double dodatnoUdaljenostAerodroma(java.lang.String icaoPocetni, java.lang.String icaoKraj, java.lang.String korisnickoIme, java.lang.String lozinka) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        org.foi.nwtis.mkolaric1.ws.AIRPWS port = service.getAIRPWSPort();
        return port.dodatnoUdaljenostAerodroma(icaoPocetni, icaoKraj, korisnickoIme, lozinka);
    }
